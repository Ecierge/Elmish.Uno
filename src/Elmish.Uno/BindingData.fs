...
type OneWayData<'model, 'T> = { Get : 'model -> 'T }


type OneWaySeqData<'model, 'msg, 'T, 'Collection, 'id when 'id : equality and 'id : not null> = {
  Get : 'model -> 'T seq
  CreateCollection : (unit -> obj) -> Dispatch<obj> -> 'T seq -> CollectionTarget<'T, 'Collection>
  GetId : 'T -> 'id
  ItemEquals : 'T -> 'T -> bool
} with
...
type OneWaySeqGroupedData<'model, 'msg, 'T, 'Collection, 'id, 'key
  when 'id : equality and 'id : not null and 'key : equality and 'key : not null> = {
  Get : 'model -> 'T seq
  CreateCollection : Dispatch<obj> -> 'T seq -> GroupedCollectionTarget<'T, 'Collection, 'key>
  GetId : 'T -> 'id
  GetKey : 'T -> 'key
...
type TwoWayData<'model, 'msg, 'T> = { Get : 'model -> 'T; Set : 'T -> 'model -> 'msg }
...
type SubModelSelectedItemData<'model, 'msg, 'id when 'id : equality and 'id : not null> = {
...
and SubModelSeqUnkeyedData<'model, 'msg, 'bindingModel, 'bindingMsg, 'vm, 'vmCollection> = {
  ...
  CreateViewModel : ViewModelArgs<'bindingModel, 'bindingMsg> -> 'vm
  CreateCollection : (unit -> obj) -> Dispatch<obj> -> 'vm seq -> CollectionTarget<'vm, 'vmCollection>
  UpdateViewModel : 'vm * 'bindingModel -> unit
...
and ValidationData<'model, 'msg, 't when 'msg : not null> = {
  BindingData : BindingData<'model, 'msg, 't>
  Validate : 'model -> string list
}


and LazyData<'model, 'msg, 'bindingModel, 'bindingMsg, 't when 'msg : not null and 'bindingMsg : not null> = {
...
and AlterMsgStreamData<'model, 'msg, 'bindingModel, 'bindingMsg, 'dispatchMsg, 't
  when 'msg : not null and 'bindingMsg : not null and 'dispatchMsg : not null> = {
  BindingData : BindingData<'bindingModel, 'bindingMsg, 't>
  Get : 'model -> 'bindingModel
...
and BindingData<'model, 'msg, 't when 'msg : not null> =
...
module BindingData =
    ...
    let baseCase (fOut : 't0 -> 't1) (fIn : 't1 -> 't0) =
      ...
      | OneWaySeqGroupedData d ->
        OneWaySeqGroupedData {
          Get = d.Get
          CreateCollection = mapCreateCollectionGrouped d.CreateCollection (GroupedCollectionTarget.mapCollection fOut)
          GetId = d.GetId
          GetKey = d.GetKey
          KeyComparer = d.KeyComparer
          ItemEquals = d.ItemEquals
        }
      | TwoWayData d -> TwoWayData { Get = d.Get >> fOut; Set = fIn >> d.Set }
      | TwoWaySeqData d ->
        TwoWaySeqData {
          Get = d.Get
          CreateCollection = mapCreateCollection d.CreateCollection (CollectionTarget.mapCollection fOut)
      ...
      | CmdData d -> CmdData { Exec = d.Exec; CanExec = d.CanExec }
      | SubModelData d ->
        SubModelData {
          GetModel = d.GetModel
      ...
      | SubModelSelectedItemData d ->
        SubModelSelectedItemData { Get = d.Get; Set = d.Set; SubModelSeqBindingName = d.SubModelSeqBindingName }

    let rec recursiveCase<'model, 'msg, 't0, 't1 when 'model : not null and 'msg : not null>
      (fOut : 't0 -> 't1)
      (fIn : 't1 -> 't0)
      ...
      | CachingData d ->
        d
        |> recursiveCase<'model, 'msg, 't0, 't1> fOut fIn
        |> CachingData
      | ValidationData d ->
        ValidationData {
  ...
  let mapModel (f : 'm -> 'm0) =
    ...
    let baseCase =
      ...
      | TwoWayData d -> TwoWayData { Get = f >> d.Get; Set = binaryHelper d.Set }
      | TwoWaySeqData d ->
        TwoWaySeqData {
          Get = f >> d.Get
          CreateCollection = d.CreateCollection
      ...
      | CmdData d -> CmdData { Exec = binaryHelper d.Exec; CanExec = binaryHelper d.CanExec }
      | SubModelData d ->
        SubModelData {
    ...
    let rec recursiveCase =
      ...
      | ValidationData d -> ValidationData { BindingData = recursiveCase d.BindingData; Validate = f >> d.Validate }
      | LazyData d ->
        LazyData {
  ...
  let mapMsgWithModel (f : 'msg -> 'model -> 'msg0) =
    let baseCase =
      ...
      | TwoWayData d -> TwoWayData { Get = d.Get; Set = fun v m -> f (d.Set v m) m }
      | TwoWaySeqData d ->
        TwoWaySeqData {
          Get = d.Get
          CreateCollection = d.CreateCollection
    ...
    let rec recursiveCase =
      ...
      | BaseBindingData d -> d |> baseCase |> BaseBindingData
      | CachingData d -> d |> recursiveCase |> CachingData
      | ValidationData d -> ValidationData { BindingData = recursiveCase d.BindingData; Validate = d.Validate }
      | LazyData d ->
        LazyData {
          BindingData = d.BindingData
          Get = d.Get
  ...
  module OneWay =
    ...
    let id<'T, 'msg when 'T : not null and 'msg : not null> : BindingData<'T, 'msg, 'T> =
      { Get = id } |> OneWayData |> BaseBindingData

    let private mapFunctions mGet (d : OneWayData<'model, 'T>) = { d with Get = mGet d.Get }
  ...
  module OneWaySeq =
    ...
    let mapMinorTypes
      ...
      =
      {
        ...
        CreateCollection =
          fun dispatch initialModel items ->
            d.CreateCollection dispatch initialModel (items |> Seq.map inMapA)
            |> CollectionTarget.mapA outMapA inMapA
    ...
    let create itemEquals getId incrementalLoader =
      {
        Get = (fun x -> upcast x)
        CreateCollection =
          fun getRootModel dispatch items ->
    ...
    let private mapFunctions mGet mGetId mItemEquals (d : OneWaySeqData<'model, 'msg, 'T, 'aCollection, 'id>) = {
      d with
          Get = mGet d.Get
  ...
  module OneWaySeqGrouped =
    ...
    let mapMinorTypes
      ...
      =
      {
        ...
        CreateCollection =
          fun dispatch items ->
            d.CreateCollection dispatch (items |> Seq.map inMapA)
            |> GroupedCollectionTarget.mapA inMapA outMapKey inMapKey
        GetId = inMapA >> d.GetId >> outMapId
    ...
    let boxMinorTypes d =
      d
      |> mapMinorTypes box (fun id -> id :> obj) (fun key -> key :> obj) unbox unbox
    ...
    let createWithComparer itemEquals getId getGrouppingKey keyComparer =
      {
        ...
        CreateCollection =
          fun _ items ->
            items.ToObservableLookup<'key, _> (keyComparer, Func<_, 'key> (getGrouppingKey))
            |> GroupedCollectionTarget.create
        ItemEquals = itemEquals
        GetId = getId
  ...
  module TwoWay =
    ...
    let id<'T when 'T : not null> : BindingData<'T, 'T, 'T> =
      { TwoWayData.Get = id; Set = Func2.id1 }
      |> TwoWayData
      |> BaseBindingData

    let private mapFunctions mGet mSet (d : TwoWayData<'model, 'msg, 'T>) = { d with Get = mGet d.Get; Set = mSet d.Set }
  ...
  module TwoWaySeq =
      ...
      =
      {
        ...
        CreateCollection =
          fun getModel dispatch items ->
            d.CreateCollection getModel dispatch (items |> Seq.map inMapA)
            |> CollectionTarget.mapA outMapA inMapA
        GetId = inMapA >> d.GetId >> outMapId
        ItemEquals = fun a1 a2 -> d.ItemEquals (inMapA a1) (inMapA a2)
        Update = d.Update
    ...
    let private mapFunctions mGet mGetId mItemEquals (d : TwoWaySeqData<'model, 'msg, 'T, 'aCollection, 'id>) = {
      d with
          Get = mGet d.Get
          GetId = mGetId d.GetId
          ItemEquals = mItemEquals d.ItemEquals
  ...
  module SubModelSelectedItem =
    ...
    let mapMinorTypes (outMapId : 'id -> 'id0) (inMapId : 'id0 -> 'id) (d : SubModelSelectedItemData<'model, 'msg, 'id>) = {
      Get = d.Get >> ValueOption.map outMapId
      Set = ValueOption.map inMapId >> d.Set
      SubModelSeqBindingName = d.SubModelSeqBindingName
    }

    let boxMinorTypes d = d |> mapMinorTypes (box >> nonNull) unbox

    let create subModelSeqBindingName =
      { Get = id; Set = Func2.id1; SubModelSeqBindingName = subModelSeqBindingName }
      |> boxMinorTypes
      |> SubModelSelectedItemData
      |> BaseBindingData
  ...
  module SubModelSeqUnkeyed =
    ...
    let mapMinorTypes
      ...
      =
      {
        ...
        CreateCollection =
          fun getModel dispatch items ->
            d.CreateCollection getModel dispatch (items |> Seq.map inMapBindingViewModel)
  ...
  module SubModelSeqKeyed =
    ...
    let mapMinorTypes
      ...
      =
      {
        ...
        UpdateViewModel =
          fun (vm, m) ->
            (inMapBindingViewModel vm, inMapBindingModel m)
            |> d.UpdateViewModel
    ...
    let boxMinorTypes d =
      d
      |> mapMinorTypes box box box (fun id -> id :> obj) unbox unbox unbox unbox
  ...
  module Validation =
    ...
    let private mapFunctions mValidate (d : ValidationData<'model, 'msg, 't>) = { d with Validate = mValidate d.Validate }
